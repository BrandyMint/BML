...
describe CloudPayments::OneTimeChargeBalance, vcr: true, openbill: true do
  ...
  let(:params)              { { amount: amount, name: 'CARDHOLDER NAME', recurrent: recurrent, cryptogram_packet: cryptogram_packet } }
  let(:form)                { BalancePaymentForm.new params }

  subject { described_class.new(account: account, form: form, ip: ip) }
